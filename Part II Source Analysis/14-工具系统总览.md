...
### 1. 写时自动校验（v0.13 + v0.14）

- **Post-write delta lint on `write_file` + `patch`**（v0.13，[#20191](https://github.com/NousResearch/hermes-agent/pull/20191)）：Python / JSON / YAML / TOML 的 in-proc 校验，写完立即把语法错回灌给 Agent
- **LSP 语义诊断**（v0.14，[#24168](https://github.com/NousResearch/hermes-agent/pull/24168) + [#25978](https://github.com/NousResearch/hermes-agent/pull/25978)）：v0.14 升级到真实语言服务器，类型错、未定义符号、缺导入等都能即时反馈
- **Per-turn file-mutation verifier footer**（v0.14，[#24498](https://github.com/NousResearch/hermes-agent/pull/24498)）：每个 turn 结束时附加"本回合写了哪些文件、变了多少行"的脚注，Agent 用来自检"我说改的真改了吗"

意义：直接把"Agent 自信地说'我改完了'但其实文件没存"这一类错误关掉。

### 2. `check_fn` 缓存（v0.12）

原章节问题 2 部分回应：

- **Memoize `get_tool_definitions()` + TTL-cache `check_fn` results**（v0.12，[#17098](https://github.com/NousResearch/hermes-agent/pull/17098)）：跨调用现在有 TTL 缓存，原章节描述的"每个 gateway 消息都重跑所有 check_fn"问题部分缓解
- **Isolate `get_tool_definitions` quiet_mode cache + dedup LCM injection**（v0.13，[#17889](https://github.com/NousResearch/hermes-agent/pull/17889)）
...
| 工具 | 版本 | 说明 |
|------|------|------|
| `video_analyze` | v0.13 | 原生视频理解（Gemini 等） |
| `video_generate`（统一接口） | v0.14 | 一个工具 + 可插拔 video provider 后端 |
| `vision_analyze` 透传像素 | v0.14 | vision-capable 模型直接拿到像素而非文本摘要 |
| `x_search` | v0.14 | 一等公民 X (Twitter) 搜索 |
| `computer_use` cua-driver | v0.14 | 非 Anthropic 模型也能做 GUI 自动化 |
| `browser_cdp` | v0.12 | 原始 DevTools Protocol 透传 |
| `kanban_*` | v0.13 | Kanban 多 Agent 协作工具集 |
| 后台 `goals` | v0.13 | `/goal` / `/subgoal` 后台支撑 |
| `x_search_tool.py` | v0.14 | X 搜索 |
| 新 web search backend (Brave / DDGS / SearXNG) | v0.13-v0.14 | 见第 17 章升级补遗（web 部分） |

### 4. 工具结果与终端输出的可插拔重写（v0.11）

- **`transform_tool_result`**（[#12972](https://github.com/NousResearch/hermes-agent/pull/12972)）：插件可以重写工具结果
- **`transform_terminal_output`**（[#12929](https://github.com/NousResearch/hermes-agent/pull/12929)）：插件可以重写终端工具输出
- **`pre_tool_call` 阻断**（[#9377](https://github.com/NousResearch/hermes-agent/pull/9377)）：插件可以否决工具执行
- **`dispatch_tool`**（[#10763](https://github.com/NousResearch/hermes-agent/pull/10763)）：插件可以反向派发工具

到 v0.14 又追加：

- **`tool_override`**（v0.14，[#26759](https://github.com/NousResearch/hermes-agent/pull/26759)）：替换内置工具
- **`ctx.llm`**（v0.14，[#23194](https://github.com/NousResearch/hermes-agent/pull/23194)）：插件用 Agent 的 LLM 调用栈

### 5. `_LEGACY_TOOLSET_MAP` 现状

原章节问题 3 未发现专门 PR；`_LEGACY_TOOLSET_MAP` 在 v0.14 仍然存在。技术债没还。

### 6. Concurrency

- **Serialize concurrent `hermes_tools` RPC calls from `execute_code`**（v0.12, [#17894](https://github.com/NousResearch/hermes-agent/pull/17894) + [#17902](https://github.com/NousResearch/hermes-agent/pull/17902)）
