...
### 1. Checkpoints v2：会话存储重写（v0.13）

**[#20709](https://github.com/NousResearch/hermes-agent/pull/20709)** 把 state.db 之外的 checkpoints 体系做了一次完整重写：

- **Single-store rewrite**：原来分散的 checkpoint 文件合并到单一存储
- **Real pruning**：之前是"删了引用但物理文件仍在"，现在有真实剪枝
- **Disk guardrails**：磁盘配额上限
- **No more orphan shadow repos**：v0.12 起就有"启动时自动 prune orphan and stale shadow repos"（[#16303](https://github.com/NousResearch/hermes-agent/pull/16303)）

### 2. Trigram FTS5（v0.12）

**[#16651](https://github.com/NousResearch/hermes-agent/pull/16651)**（@alt-glitch）：

- FTS5 用 trigram tokenizer 重建索引，原来对中文 / 日文 / 韩文等 CJK 内容退化成 `LIKE` fallback
- v0.12 还把 `tool_name` + `tool_calls` 纳入 FTS5 索引，并附带迁移（[#16914](https://github.com/NousResearch/hermes-agent/pull/16914)）

部分回应原章节问题 5："FTS5 查询注入的边界情况" 在 v0.12 [#16915](https://github.com/NousResearch/hermes-agent/pull/16915) 中加固了下划线词的 quote。

### 3. 会话与状态的持久化稳健性

- **Auto-prune old sessions + VACUUM state.db at startup**（v0.11，[#13861](https://github.com/NousResearch/hermes-agent/pull/13861)）：直接回应原章节问题 4
- **SQLite: fall back to `journal_mode=DELETE` on NFS / SMB / FUSE**（v0.14，[#22043](https://github.com/NousResearch/hermes-agent/pull/22043)）：网络挂载下 `/resume` 不再失败。直接回应原章节问题 3
- **Atomic restart markers + Windows runtime-lock offset**（v0.13，[#18179](https://github.com/NousResearch/hermes-agent/pull/18179)）
- **Auto-resume interrupted sessions after gateway restart**（v0.13，[#21192](https://github.com/NousResearch/hermes-agent/pull/21192)）：gateway bounce mid-agent、`/update`、source-file reload 等情况下会话自动接续
- **Preserve session when switching personality** (v0.14, [#20942](https://github.com/NousResearch/hermes-agent/pull/20942))
- **Preserve assistant metadata when branching sessions**（v0.13，[#18222](https://github.com/NousResearch/hermes-agent/pull/18222)）
- **Preserve symlinks during atomic file writes**（v0.12，[#16980](https://github.com/NousResearch/hermes-agent/pull/16980)）
- **Preserve home-channel thread targets across restart notifications**（v0.13，[#19271](https://github.com/NousResearch/hermes-agent/pull/19271)）

### 4. 与会话紧密耦合的"接管"机制

- **`/handoff` 实时移交会话**（v0.14，[#23395](https://github.com/NousResearch/hermes-agent/pull/23395)）：跨 model / persona / profile 把当前会话整体迁移
- **Expose `HERMES_SESSION_ID` env var to agent tools**（v0.14，[#23847](https://github.com/NousResearch/hermes-agent/pull/23847)）：会话 ID 现在以环境变量形式暴露给工具进程
- **API server `X-Hermes-Session-Key` header**（v0.13，[#20199](https://github.com/NousResearch/hermes-agent/pull/20199)）：长记忆 provider 用一个稳定 session 标识符

### 5. 与"修剪粒度"相关的小进展

原章节问题 4 "会话修剪不考虑重要性"未根治；但 v0.12 [#13861](https://github.com/NousResearch/hermes-agent/pull/13861) 把"启动 prune + vacuum"作为默认起手，避免无界生长。
