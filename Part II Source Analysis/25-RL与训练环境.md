...
flowchart LR
    subgraph prepare ["准备"]
        START["collect_trajectory(item)"] --> UUID["Generate task_id"]
        UUID --> TOOLS["Get group-level tools"]
        TOOLS --> MSGS["Build initial messages"]
    end
    subgraph execute ["执行"]
        MSGS --> MODE{"Phase 1 or 2?"}
        MODE -->|"Phase 1<br/>OpenAI"| P1["AgentLoop +<br/>server"]
        MODE -->|"Phase 2<br/>VLLM"| P2["AgentLoop +<br/>ManagedServer"]
        P1 --> RESULT["AgentResult"]
        P2 --> RESULT
    end
    subgraph reward ["奖励计算"]
        RESULT --> CHECK{"turns gt 0?"}
        CHECK -->|No| ZERO["reward = 0.0"]
        CHECK -->|Yes| CTX["ToolContext"]
        CTX --> RW["compute_reward()"]
        RW --> CLEAN["cleanup()"]
        CLEAN --> BUILD["ScoredDataItem"]
        ZERO --> BUILD
    end
```
