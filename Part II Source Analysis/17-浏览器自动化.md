...
如果安全模块导入失败，两处回退策略并不一致：`is_safe_url` 回退为 `lambda: False`，属于 **fail-closed**（全部阻断）；`check_website_access` 回退为 `lambda: None`，更接近 **fail-open**（不额外阻断）。这个不一致的默认策略值得注意。
...
### 1. CDP supervisor + 180x 加速（v0.12 + v0.14）

- **`browser_cdp` 原始 DevTools Protocol 透传**（v0.12，[#12369](https://github.com/NousResearch/hermes-agent/pull/12369)）
- **CDP supervisor — dialog 检测 / 响应 / cross-origin iframe eval**（v0.12，[#14540](https://github.com/NousResearch/hermes-agent/pull/14540)）
- **`browser_console` 评估通过 supervisor 的持久 CDP WS（180x faster）**（v0.14，[#23226](https://github.com/NousResearch/hermes-agent/pull/23226)）：之前每次 console 评估都新起一个 DevTools 会话，现在共用一条持久连接

意义：原章节性能讨论里"页面交互延迟"问题在 v0.14 直接降一个数量级。

### 2. 本地 Chromium 自动回退（v0.12）

部分回应原章节问题 3 "CamoFox 单点故障"：

- **Auto-spawn local Chromium for LAN/localhost URLs**（v0.12，[#16136](https://github.com/NousResearch/hermes-agent/pull/16136)）：配置了云端 provider 时，对本地 URL 自动切到本地 Chromium，避免"云端浏览器访问不到 LAN"
- **Allow explicit CDP override without local agent-browser**（v0.13，[#19670](https://github.com/NousResearch/hermes-agent/pull/19670)）
- **Support externally managed Camofox sessions**（v0.14，[#24499](https://github.com/NousResearch/hermes-agent/pull/24499)）

### 3. 安全侧

- **Browser enforces cloud-metadata SSRF floor in hybrid routing**（v0.13 P0，[#21228](https://github.com/NousResearch/hermes-agent/pull/21228)）：堵掉一个混合路由下的元数据 SSRF 路径
- **Skip browser download when system chromium exists**（v0.14，[#25317](https://github.com/NousResearch/hermes-agent/pull/25317)）

### 4. Web 工具的多后端拆分

虽然不在浏览器主线，但与浏览器协同的 web 工具有显著调整：

- **Per-capability backend selection — search / extract split**（v0.13，[#20061](https://github.com/NousResearch/hermes-agent/pull/20061)）：search / extract 各自挑 backend
- **SearXNG 原生 search-only backend**（v0.13，[#20823](https://github.com/NousResearch/hermes-agent/pull/20823)）
- **Brave Search (free tier) + DDGS providers**（v0.14，[#21337](https://github.com/NousResearch/hermes-agent/pull/21337)）
- **Bearer auth header for Tavily `/crawl` endpoint**（v0.14，[#24658](https://github.com/NousResearch/hermes-agent/pull/24658)）
- `plugins/web/` 截至 v0.14 含 brave_free / ddgs / exa / firecrawl / parallel / searxng / tavily / xai 八个后端
