...
%%{init: {'theme': 'neutral', 'themeVariables': {'background': '#ffffff', 'primaryColor': '#f5f5f5', 'primaryTextColor': '#000000', 'primaryBorderColor': '#333333', 'lineColor': '#444444', 'textColor': '#000000', 'mainBkg': '#f5f5f5', 'nodeBorder': '#333333', 'clusterBkg': '#fafafa', 'clusterBorder': '#888888', 'edgeLabelBackground': '#ffffff'}}}%%
flowchart TD
    A["handle_function_call()"] --> B["coerce_tool_args: 类型修正"]
    B --> C{"_AGENT_LOOP_TOOLS?"}
    C -->|Yes| D["返回错误, 由 agent loop 处理"]
    C -->|No| E["pre_tool_call_hook: 插件拦截"]
    E --> F{"被阻断?"}
    F -->|Yes| G["返回 block_message"]
    F -->|No| H["notify_other_tool_call: 重置读循环计数"]
    H --> I{"execute_code?"}
    I -->|Yes| J["dispatch + enabled_tools 沙箱工具列表"]
    I -->|No| K["dispatch + task_id + user_task"]
