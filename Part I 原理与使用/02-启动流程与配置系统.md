...
flowchart TD
    ...
    CHOICE -->|"curl 一键脚本"| CURL["curl ... setup-hermes.sh"]
    CHOICE -->|"pip 手动安装"| PIP["pip install hermes-agent"]
    CHOICE -->|"包管理器"| PKG["brew / nix"]
    CURL --> SCRIPT["setup-hermes.sh"]
    SCRIPT --> VENV["创建 Python venv"]
    VENV --> PIPINSTALL["pip install hermes-agent"]
    PIPINSTALL --> ENTRY["注册 hermes 命令"]
    PIP --> ENTRY
    PKG --> MANAGED["设置 HERMES_MANAGED 标记"]
    MANAGED --> ENTRY
    ENTRY --> SETUP["hermes setup 向导"]
    SETUP --> PROVIDER["选择 Provider + 模型"]
    PROVIDER --> KEYS["配置 API 密钥"]
    KEYS --> DONE["就绪"]
```
...
flowchart TD
    START["hermes 启动"] --> CHECK{"check_config_version()"}
    CHECK -->|"current lt latest"| MIGRATE["执行版本迁移"]
    CHECK -->|"current gte latest"| READY["配置就绪"]
    MIGRATE --> V4["v3-v4: tool_progress 从 .env 迁移到 config"]
    V4 --> VN["v4-vN: 逐版本增量迁移"]
    VN --> BUMP["config._config_version = latest"]
    BUMP --> SAVE["save_config()"]
    SAVE --> ENVCHECK{"检查新增 env vars"}
    ENVCHECK -->|"有未设置的新变量"| PROMPT["提示用户配置"]
