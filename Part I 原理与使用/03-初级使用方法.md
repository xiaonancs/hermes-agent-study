...
flowchart TD
    START["curl 下载 install.sh"] --> DETECT{"检测操作系统"}
    DETECT -->|"Linux"| LINUX["检查 Python 3.10+"]
    DETECT -->|"macOS"| MACOS["检查 Python 3.10+"]
    DETECT -->|"WSL2"| WSL["检查 Python 3.10+"]
    DETECT -->|"Termux"| TERMUX["检查 Python 3.10+"]
    LINUX --> PYCHECK{"Python 版本满足?"}
    MACOS --> PYCHECK
    WSL --> PYCHECK
    TERMUX --> PYCHECK
    ...
    INSTALL_PY --> VENV
    VENV --> PIP["pip install hermes-agent"]
    PIP --> SYMLINK["创建 hermes 命令软链接"]
    SYMLINK --> PATHRC["写入 PATH 到 .bashrc/.zshrc"]
    PATHRC --> RELOAD["提示 source 刷新"]
    RELOAD --> SETUP["运行 hermes setup"]
    SETUP --> DONE["安装完成"]
```
...
flowchart TD
    ...
    BUDGET -->|"免费/极低"| FREE["Nous Portal 免费额度"]
    BUDGET -->|"低预算"| LOW{"主要用途?"}
    BUDGET -->|"中等预算"| MID{"模型偏好?"}
    BUDGET -->|"不限预算"| HIGH{"场景需求?"}
    LOW -->|"编程"| DEEPSEEK["DeepSeek V4 -- 约 $2/月"]
    LOW -->|"中文任务"| QWEN["Qwen-Max -- 约 $5/月"]
    LOW -->|"固定预算"| MINIMAX["MiniMax -- $10/月"]
    MID -->|"OpenAI 系"| OPENROUTER["OpenRouter -- $10-30/月"]
    MID -->|"Anthropic 系"| CLAUDE["Claude Sonnet -- 约 $30/月"]
    MID -->|"超长上下文"| KIMI["Kimi K2.5"]
    HIGH -->|"最强推理"| GPT5["GPT-5 / Claude Opus"]
    HIGH -->|"企业合规"| BEDROCK["AWS Bedrock"]
    HIGH -->|"已有 Copilot"| COPILOT["GitHub Copilot -- 复用订阅"]
```
...
| **中级** | 第四章 SystemPrompt 组装 | 理解 Agent 如何"思考" |
| **中级** | 第五章 Provider 与 API 模式 | 多模型切换和成本优化 |
| **中级** | 第六章 记忆系统 | 跨会话知识持久化 |
| **高级** | 第七章 Skill 系统 | 创建自定义 Skill 扩展能力 |
| **高级** | 第八章 自进化引擎 | 让 Agent 自我改进 |
