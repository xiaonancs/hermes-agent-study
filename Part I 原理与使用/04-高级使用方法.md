...
> **导读**：前三章覆盖了 Hermes Agent 的核心循环、启动流程和 SystemPrompt 组装机制。本章从使用者视角出发，系统梳理 Hermes 的十二项高级能力——从 Cron 定时自动化到 Voice 语音交互，从 Skill 生态到多平台 Gateway 部署。每一节都遵循"概念 → 配置 → 典型用法 → 注意事项"的结构，力求让读者在理解原理的同时能直接上手操作。
>
...
flowchart TD
    subgraph create ["1. 创建阶段"]
        USER["用户定义 cron 任务"] --> PARSE["解析 cron 表达式"]
        PARSE --> STORE["持久化到调度表"]
    end
    subgraph schedule ["2. 调度阶段"]
        STORE --> TIMER["系统定时器触发"]
        TIMER --> SCRIPT{"有 --script 参数?"}
    end
    subgraph execute ["3. 执行阶段"]
        SCRIPT -->|"是"| RUNSCRIPT["执行预处理脚本"]
        RUNSCRIPT --> INJECT["脚本 stdout 注入 Agent 上下文"]
        SCRIPT -->|"否"| AGENT["启动全新 Agent 会话"]
        INJECT --> AGENT
        AGENT --> TOOLS["Agent 自主调用工具"]
        TOOLS --> RESULT["生成结果文本"]
    end
    subgraph deliver ["4. 投递阶段"]
        RESULT --> SILENT{"输出包含 SILENT?"}
        SILENT -->|"是"| SUPPRESS["静默，不发送"]
        SILENT -->|"否"| TARGET["投递到目标通道"]
        TARGET --> ORIGIN["origin -- 原始来源"]
        TARGET --> TG["Telegram"]
        TARGET --> DC["Discord"]
        TARGET --> SL["Slack"]
        TARGET --> LOCAL["local -- 本地日志"]
    end
```
...
flowchart LR
    ...
    subgraph install ["安装"]
        BROWSE --> INSTALL["hermes skills install"]
        WELLKNOWN --> INSTALL
        INSTALL --> REVIEW{"安全审查"}
        REVIEW -->|"官方来源"| AUTO["自动通过"]
        REVIEW -->|"第三方"| FORCE["需要 --force"]
    end
    subgraph use ["使用"]
        AUTO --> LOAD["Skill 加载到 Agent 上下文"]
        FORCE --> LOAD
        LOAD --> INVOKE["Agent 使用 Skill 工具"]
    end
    subgraph create ["创建"]
        INVOKE --> EXTRACT["从交互中提取"]
        EXTRACT --> PACKAGE["打包为 Skill"]
        PACKAGE --> PUBLISH["发布到 skills.sh"]
    end
```
...
flowchart TD
    subgraph approval ["命令审批层"]
        CMD["Agent 生成命令"] --> CHECK{"命令危险性评估"}
        CHECK -->|"安全命令"| PASS["自动放行"]
        CHECK -->|"危险命令"| ASK["用户确认"]
        ASK --> ONCE["once -- 仅本次"]
        ASK --> SESSION["session -- 本次会话"]
        ASK --> ALWAYS["always -- 永久允许"]
        ASK --> DENY["deny -- 拒绝"]
    end
    subgraph sandbox ["沙箱隔离层"]
        ...
        SESSION --> BACKEND
        ALWAYS --> BACKEND
        BACKEND -->|"local"| LOCAL["本地 Shell 直接执行"]
        BACKEND -->|"docker"| DOCKER["Docker 容器内执行"]
        BACKEND -->|"ssh"| SSH["远程服务器执行"]
        BACKEND -->|"modal"| MODAL["Serverless 环境执行"]
    end
    subgraph access ["访问控制层"]
        LOCAL --> FS["完整文件系统访问"]
        DOCKER --> MOUNT["仅挂载目录可访问"]
        SSH --> REMOTE["远程文件系统"]
...
下图给出了本章十二项能力的整体关系图。核心 Agent 循环位于中心，向外辐射出三层能力圈：内层是每次会话都可能使用的交互增强能力，中层是需要一次性配置的系统级能力，外层是面向特定场景的扩展能力。
...
下一章将深入 Provider 与 API 模式的底层机制，解析 Hermes 如何在四种不同的 API 协议之间无缝切换。
