...
> 本文是全书导论，系统性回答三个核心问题：Hermes Agent 的机制本质是什么？它为什么会火爆？核心逻辑和设计原理是什么？
...
flowchart TD
    TASK["复杂任务完成 (5+ 工具调用)"] --> EVAL{"Agent 评估: 值得记住吗?"}
    EVAL -->|Yes| CREATE["skill_manage(action=create)"]
    EVAL -->|No| DONE["结束"]
    CREATE --> WRITE["写入 SKILL.md 到 ~/.hermes/skills/"]
    WRITE --> INDEX["更新 skills_list() 索引"]
    INDEX --> FUTURE["后续类似任务自动加载"]
    
    FUTURE --> USE["Agent 使用已有 Skill"]
    USE --> BETTER{"发现更好的方法?"}
    BETTER -->|Yes| PATCH["skill_manage(action=patch)"]
    BETTER -->|No| KEEP["保持不变"]
    PATCH --> WRITE
```
...
第 10-18 章深入工具系统和执行引擎：47 个工具如何注册和分发、6 种沙箱后端如何选择、MCP 集成如何工作、Skill 如何自动创建和进化。这一部分揭示了 Agent 的"手和脚"。

### Part II 源码分析

第 10-23 章逐模块拆解实现细节：上下文压缩、会话持久化、47 个工具的注册分发、6 种沙箱后端、9,798 行的 Gateway 网关、18 个平台适配器、插件系统、安全防御和 RL 训练环境。

### Part III 对比分析

第 24-28 章是本书的特色：从代码层面对比 Hermes、OpenClaw 和 EvoMap Evolver 三个项目，分析架构继承关系、代码相似度和抄袭争议，最后讨论开源伦理和 AI 时代的代码洗稿问题。
