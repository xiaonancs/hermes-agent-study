...
>
> **基线说明**：正文主线以 Hermes Agent v0.9.0（2026-04-13）为基线撰写。v0.9.0 之后的体系级演进（v0.10 → v0.14，30 天 5 个版本）请直接读专题文章 [近期重大升级 — v0.10 到 v0.14](近期重大升级-v0.10到v0.14.md)，本文末尾 § 11 也给出一个"基线之后"的快速对照。
...
## 11. 基线之后：v0.10 → v0.14 的三条主线（提要）

> 本节是与原研究"未来不会回头改"的快速对照；详细内容见 [近期重大升级 — v0.10 到 v0.14](近期重大升级-v0.10到v0.14.md)。

### 11.1 数据基线对照

| 维度 | v0.9.0 | v0.14 后 | 变化 |
|------|--------|----------|------|
...
| 消息平台数 | 18 | 22 | +22% |
| 可选 Skill 子目录 | 14 | 82 | +485% |
| `run_agent.py` 行数 | 11,487 | **4,410** | **-61.6%** |
| `gateway/run.py` 行数 | 9,798 | 18,556 | +89% |
| `cli.py` 行数 | 10,033 | 15,089 | +50% |
| `hermes_cli/main.py` 行数 | 6,383 | 13,981 | +119% |

注意 `run_agent.py` 反向"缩水"——这是 v0.11 Transport ABC 抽离的直接结果。

### 11.2 三条主线一句话

1. **底座坚固化**：Transport ABC（v0.11）+ Ink TUI（v0.11）+ Checkpoints v2（v0.13）+ PyPI 包（v0.14）+ 原生 Windows beta（v0.14）+ 两次冷启动连削（v0.12 -57%，v0.14 -19s）。`run_agent.py` 从巨型主循环瘦成调度层
2. **自治层**：Curator（v0.12 后台 7 天周期维护技能库）+ Kanban（v0.13 多 profile 协作板）+ `/goal` `/subgoal`（v0.13/v0.14 Ralph 循环）+ `/handoff` `/steer`（v0.11/v0.14 实时干预），从"对话式单 Agent"演化到"持续运行的多 Agent 系统"
3. **接入扩张**：Tool Gateway（v0.10）+ 4 个新平台（v0.11-v0.14）+ Pluggable Provider（v0.13）+ Pluggable Platform（v0.12）+ OpenAI 兼容代理（v0.14），把 Hermes 反过来变成"任意 OAuth → 任意 OpenAI 兼容客户端"的中继

### 11.3 与原研究判断的关系

- **"Hermes 是 OpenClaw 的 Python 重实现 + EvoMap 自进化嫁接"** —— 仍成立。Curator 是 review-fork 的工程化延伸，Kanban 是多 Agent 协作板，都不削弱原判断
- **"6 大子系统"模型** —— 主体仍成立，但需加一个新子系统：**自治层（Curator + Kanban + Goals）**
- **"三种 API 模式"** —— 仍成立，但物理上由 `agent/transports/` 下若干 Transport 类承载，而非 `run_agent.py` 内联
