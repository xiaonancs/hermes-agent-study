...
这些问题并不影响 Hermes Agent 作为当前开源 AI Agent 产品中完成度较高的一类实现被认真研究。它在"能跑"到"能用"之间补上了大量工程环节，值得 Agent 开发者深入分析。

---

## 升级补遗（v0.10 → v0.14）

> 全景层面的核心变化。完整版本对照见 [近期重大升级 — v0.10 到 v0.14](../近期重大升级-v0.10到v0.14.md)。

### 数据基线对照

| 维度 | 本章原写（v0.9.0） | v0.14 后 main |
|------|----------------------|----------------|
| Python 文件数 | 886 | 1,898 |
| Python 总行数 | 411,492 | 911,835 |
| Markdown 文件数 | 556 | 1,374 |
| 测试文件数 | 577 | 1,266 |
| 内置 Skill 类别 | 26 | 25 顶层目录（含 88 二级子目录） |
| 可选 Skill 子目录 | 14 | 82 |
| 消息平台数 | 18 | 22 |
| `run_agent.py` 行数 | 11,487 | **4,410**（Transport ABC 抽离） |
| `gateway/run.py` 行数 | 9,798 | 18,556 |
| `cli.py` 行数 | 10,033 | 15,089 |
| `hermes_cli/main.py` 行数 | 6,383 | 13,981 |

### 顶层目录新增

| 目录 | 加入版本 | 说明 |
|------|----------|------|
| `agent/transports/` | v0.11 | Transport ABC + 4 个具体实现（Anthropic / ChatCompletions / Responses-API / Bedrock）+ Codex app-server + 工具 MCP server |
| `ui-tui/` | v0.11 | React/Ink 重写的交互式 TUI 前端（TypeScript） |
| `tui_gateway/` | v0.11 | TUI 的 Python JSON-RPC 后端 |
| `hermes_cli/proxy/` | v0.14 | OpenAI 兼容本地代理（把任意 OAuth 提供商伪装成 OpenAI 端点） |
| `plugins/platforms/` | v0.12 | 平台插件化目录，已托管 8 个平台（discord / google_chat / irc / line / mattermost / ntfy / simplex / teams） |
| `plugins/model-providers/` | v0.13 | Provider 插件化目录（28 个内置 provider 配置） |
| `agent/curator.py` (1,781 行) | v0.12 | 自治型技能库维护后台 |
| `hermes_cli/kanban*.py` (~2,800 行) | v0.13 | 多 Agent 协作 Kanban |
| `hermes_cli/goals.py` (762 行) | v0.13 | `/goal` `/subgoal` Ralph 循环 |

### 子系统对原模型的扩展

原章节的"6 大子系统模型（Agent Loop / Prompt / Tool / Session / Memory / Gateway）"在 v0.14 后需要追加一个新子系统：

- **子系统 7：自治层（Autonomy Layer）**——由 `agent/curator.py` + `hermes_cli/kanban*.py` + `hermes_cli/goals.py` 构成。Curator 是后台 cron 周期维护，Kanban 是多 profile 多 worker 持久协作板，Goals 提供 Ralph 循环原语。这一层的特点是"用户不在场也能运行"。

### 设计哲学侧的新增项

- **Pluggable everything**：Provider / Platform / Memory / Image Gen / Video Gen / Web Search / TTS / Tool 全部进入可插拔阶段，"内置 vs 插件"的边界开始模糊
- **OAuth 中继器**：v0.14 的 `hermes proxy` 把 Hermes 从"消费 OAuth 的 Agent"反过来变成"对外提供 OpenAI 兼容协议的中继"
- **PyPI + 原生 Windows**：v0.14 起 `pip install hermes-agent && hermes` 可用；Windows 进入 early beta，结束 WSL-only 时代
- **冷启动连削**：v0.12 cut 约 57%，v0.14 再 cut 约 19s，反映对"研究产物 → 日用工具"的工程化倾向
