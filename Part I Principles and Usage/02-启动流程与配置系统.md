...
v0.14 起 Hermes Agent 是一个真正的 PyPI 包：
...
不再需要 clone 仓库或运行 shell 安装脚本。Wheel 内置了 Ink TUI bundle 和 shell launcher。

配套出现的"瘦身组件"：

- **lazy-deps 框架**：Slack / Matrix / Feishu / DingTalk 适配器 SDK、hindsight client、codex app-server、Pixverse / Camofox / image-gen / voice / TTS provider 全部首次使用时再装，干净的 `pip install hermes-agent` 不再下载这些
- **`[all]` extras 重排**：`[all]` 现在不再覆盖 lazy-deps 范围
- **分层安装回退**：某个 wheel 在当前平台拒绝时自动降级到下一档
- **supply-chain advisory 检查**：每次安装跑一遍漏洞扫描

### `hermes -z` 一次性模式（v0.12）

非交互式 one-shot：

```bash
hermes -z "<prompt>"
hermes -z --model <model> --provider <provider> "<prompt>"
HERMES_INFERENCE_MODEL=<model> hermes -z "<prompt>"
```

适合 cron / CI / 一次性脚本场景，跳过欢迎横幅、跳过持久化会话头。

### `hermes update --check` 预检（v0.12）

`hermes update --check` 在真正动手前先确认更新是否可用。配套：opt-in 预更新 HERMES_HOME 备份（默认关闭，启用后写 `backups/` 目录，自动排除 `checkpoints/` 和 SQLite WAL/SHM/journal sidecar）。

### 原生 Windows（v0.14 early beta）

第一次在 `cmd.exe` 与 PowerShell 下原生跑通：

- PowerShell installer 处理 MinGit 自动安装、Microsoft Store python stub 检测、前台 Ctrl+C 拦截
- 不再强制 WSL2
- 仍是 "early beta"——v0.14 单一窗口内已经合入 ~40 个 Windows-only fix

### 配置系统的相关变化

- **`config.yaml` 优先于 `.env`**（v0.13）：agent / display / timezone 等设置以 config.yaml 为准
- **`hermes memory reset` 子命令**（v0.11）：在 CWD 系列环境变量被废弃的同时给出新入口
- **Profile 仓库化分发**（v0.14）：`shareable profile distributions via git`，profile 可以从 git 仓库克隆
- **Nous OAuth 跨 profile 共享 token store**（v0.13）：一次登录、所有 profile 继承
- **`hermes -p <profile> --no-skills`**（v0.13）：创建空 profile 不带任何 skill
- **`config.yaml` 单一 CWD 源**（v0.11，#11029）：废弃 `.env` 的 CWD 变量，并配 `hermes memory reset`

### 启动性能（冷启动连削两刀）

| 优化 | 版本 | 收益 |
|------|------|------|
| Lazy agent init（TUI） | v0.12 | 可见冷启动 -57% |
| Lazy import OpenAI / Anthropic / Firecrawl / account_usage | v0.12 | 启动 import 时间显著下降 |
| mtime 缓存 `load_config()` / `read_raw_config()` | v0.12 | 反复启动不再重读 yaml |
| Memoize `get_tool_definitions()` + TTL-cache `check_fn` | v0.12 | tools 列表生成不再重算 |
| Skills cache + lazy Feishu + 启动跳过 Nous HTTP | v0.14 | 冷启动 -19s |
| Skip eager plugin discovery on known built-in subcommands | v0.14 | `hermes status` / `hermes doctor` 等子命令更快 |
| `models.dev` cache-first 查询 | v0.14 | 网络可达时不强求联网 |
| `hermes tools` 全平台界面：14s → <1.5s | v0.14 | Nous auth + .env 加载结果共享缓存 |
| `chat -q` 单次查询跳过欢迎横幅 | v0.14 | 即用即出 |
| `hermes doctor` 并行 API 连通性检查 + 关闭 IMDS | v0.14 | doctor 更快 |
