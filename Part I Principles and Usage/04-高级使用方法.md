...
## 升级补遗（v0.10 → v0.14）：四个新的高级场景

> 本节是对原章节"高级使用"主线的延伸。v0.10 之后 Hermes 在"用户已经能用，但希望更长远"层面追加了四个大题目。完整版本对照见 [近期重大升级 — v0.10 到 v0.14](../近期重大升级-v0.10到v0.14.md)。

### A. 后台自治：`hermes curator`（v0.12）

技能库的自动维护后台。默认 7 天周期，挂在 gateway cron ticker 上：

```bash
hermes curator status     # 按使用频次排名 skill
hermes curator run         # 同步执行一轮（v0.13 起不再轮询）
hermes curator archive <skill_name>
hermes curator prune
hermes curator list-archived
```

- 评分、合并、清理 skill；产出 `logs/curator/REPORT.md` + `run.json`
- 防御：默认拒绝改写 bundled / hub 技能（v0.13 用 frontmatter name 闭合一个绕过）
- 模型选择：统一在 `auxiliary.curator`，从 `hermes model` 或 Dashboard 配置
- 入口：`agent/curator.py` (1,781 行) + `hermes_cli/curator.py`

### B. 多 Agent 协作：`hermes kanban`（v0.13）

跨 profile、跨 worker 的耐久 Kanban 板。这是 v0.13 的旗舰：

```bash
hermes kanban create <board>
hermes kanban add <board> <task>
hermes kanban claim <task>
hermes kanban swarm <board>   # 起多个 worker
hermes kanban diagnose <board>
```

可靠性设计：

- Worker 心跳 / reclaim / 僵尸检测 / 重试预算 / 幻觉网关
- `kanban_list` + `kanban_unblock` 工具（v0.14）
- 卡片作者注入防护（v0.14 关闭了一个 #22109 类绕过）
- 仪表板：拖拽、内嵌创建、平台首页通知开关、按租户过滤

入口：

- `hermes_cli/kanban.py`（2,762 行）+ `kanban_db.py` + `kanban_decompose.py` + `kanban_specify.py` + `kanban_swarm.py` + `kanban_diagnostics.py`
- `tools/kanban_tools.py`（Agent 端工具）
- `plugins/kanban/`（仪表板插件）
- `optional-skills/creative/kanban-video-orchestrator/`（社区示例）

意义：在原章节"高级使用"框架内，Kanban 是一个新维度——不再是"我让一个 Agent 做完一件事"，而是"我开一块板子，丢若干任务，由若干 worker 互相协作完成"。

### C. Ralph 循环：`/goal` `/subgoal` `/steer` `/handoff`

四个新的会话内原语：

| 原语 | 版本 | 用法 | 作用 |
|------|------|------|------|
| `/steer <text>` | v0.11 | 在 agent 跑工具时直接键入 | 下一次 tool call 后注入提示，不打断当前 turn、不破缓存 |
| `/goal <text>` | v0.13 | 锁定持续目标 | Agent 进入 Ralph 循环，按 judge 判定"是否完成"为止 |
| `/subgoal <text>` | v0.14 | 给运行中的 `/goal` 追加判据 | judge 折算到原循环，不重启 |
| `/handoff <target>` | v0.14 | 把当前会话实时移交 | 全部消息 / 工具调用 / 上下文交给目标 model / persona / profile |

实现入口：`hermes_cli/goals.py`（762 行）。

注意：v0.14 release 明确写到 `/goal` checklist + `/subgoal` 的初版设计被回滚（#23813），`/subgoal` 以简化版在 #25449 重新登场。说明 judge 设计还在迭代中。

### D. 跨工具 / 跨平台的"接入扩张"

| 能力 | 版本 | 简述 |
|------|------|------|
| `hermes proxy` | v0.14 | OpenAI 兼容本地代理。Codex / Aider / Cline / Continue 可借住 Hermes 已 OAuth 的 Claude Pro / ChatGPT Pro / SuperGrok |
| Nous Tool Gateway | v0.10 | Nous Portal 付费订阅一键打开 web search / image gen / TTS / browser，零额外 API key |
| Webhook 直发模式 | v0.11 | 不经 LLM，零成本推送通知（uptime / alerting / 事件流） |
| `no_agent` cron | v0.13 | cron 任务跳过 Agent 直接跑脚本；空 stdout 静默，非空逐字交付（watchdog 模式） |
| `context_from` cron 链接 | v0.12-v0.13 | 多 cron job 输出串联 |
| Per-job `enabled_toolsets` cron | v0.11 | 单 cron job 限制工具集，截 token 与成本 |
| Per-job `workdir` cron | v0.12 | 项目感知的 cron 执行 |
| `[[as_document]]` skill 媒体路由指令 | v0.13 | skill 强制 gateway 在支持的平台上把输出当文档发 |
| `hermes -z <prompt>` 一次性模式 | v0.12 | 非交互式，适合 CI / 脚本 |

### autoresearch 在新形态下的位置（保留原章节小节）

- `autoresearch` 仍然是原章节里 Hermes "研究型 Agent 工作流" 的代表场景
- v0.13 / v0.14 的 `/goal` Ralph 循环、`watchers` optional skill（v0.14 RSS / HTTP JSON / GitHub 轮询）、Kanban 多 worker 协作，都可以作为 `autoresearch` 的演化路径——三者拼起来近似一个"长跑研究助手"的雏形
