...
graph LR
    subgraph init ["初始化"]
        ...
        INIT --> REG["add_provider:<br/>Builtin + External"]
        REG --> SYS["build_system_prompt()"]
    end
    subgraph loop ["对话循环"]
        direction TB
        TURN["on_turn_start"] --> PRE["prefetch_all(query)"]
        PRE --> API["LLM API 调用"]
        API --> TOOL{"记忆工具调用?"}
        TOOL -->|Yes| HANDLE["handle_tool_call"]
        TOOL -->|No| SYNC["sync_all"]
        HANDLE --> SYNC
    end
    subgraph post ["后处理"]
        direction TB
        QUEUE["queue_prefetch_all"] --> NUDGE{"nudge 间隔?"}
        NUDGE -->|Yes| REVIEW["后台审查记忆"]
        NUDGE -->|No| NEXT["等待下轮"]
        REVIEW --> NEXT
        NEXT -->|"会话结束"| END["shutdown_all"]
    end
    SYS --> TURN
    SYNC --> QUEUE
    NEXT -.->|"循环"| TURN
