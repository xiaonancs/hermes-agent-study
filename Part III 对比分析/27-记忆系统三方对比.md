...
记忆系统是 AI Agent 实现跨会话连续性的核心基础设施。一个 Agent 能否记住用户的偏好、学习新的技能、从过去的错误中汲取教训，完全取决于其记忆架构的设计。本章将对 Hermes Agent、EvoMap Evolver 和 OpenClaw 三个项目的记忆系统进行系统性对比分析，揭示三种不同设计哲学下的工程权衡。

三个项目的记忆系统分别代表了三种典型范式：Hermes 采用"策展式记忆"（Curated Memory），由 Agent 主动管理有限容量的记忆条目；Evolver 采用"进化式记忆"（Evolutionary Memory），以基因、胶囊和叙事日志记录系统的演化轨迹；OpenClaw 则采用"会话式记忆"（Session-scoped Memory），依赖会话上下文和技能系统实现知识复用。
...
sequenceDiagram
    ...
    U->>A: 用户输入
    A->>M: memory tool 写入 (更新磁盘, 不更新提示词)
    A->>M: session_search 按需检索历史

    Note over S,M: Evolver: 叙事注入模式
    M->>A: 进化循环前读取 narrative 最新 N 条
    M->>A: Gene Selector 按 Jaccard 匹配基因
...
| 对比维度 | Hermes Agent | EvoMap Evolver | OpenClaw |
|---------|-------------|----------------|----------|
| **设计哲学** | 策展式 (有限 + 精选) | 进化式 (适者生存) | 会话式 (用完即弃) |
| **持久化后端** | SQLite (WAL) + Markdown | JSON + JSONL + Markdown | JSONL + 文件系统 |
| **并发安全** | 文件锁 + 原子替换 + WAL<br/>+ 应用层 jitter 重试 | 原子写入 (tmp + rename) | 文件系统级 |
| **安全扫描** | 注入检测 (12 种模式)<br/>+ 不可见字符检测 | 无 | 无 |
| **上下文效率** | 冻结快照 (KV-Cache 友好)<br/>+ 渐进式展开 | 截断窗口 | 按需加载 |
| **LLM 辅助** | FTS5 摘要 + 预取流水线 | 进化提示词组装 | 无 |
| **插件生态** | 8 个外部记忆提供者 | 无 | 无 |
...
OpenClaw 的会话作用域记忆是三者中最轻量的方案，适合无状态或弱状态的 Agent 场景。其简洁性是一种设计选择而非缺陷——在某些安全敏感的场景中，"不记忆"反而是一种特性。

三个项目的记忆系统对比，本质上反映了 AI Agent 领域的一个核心张力：通用性与专注度、功能完备与工程简洁之间的权衡。没有一种方案在所有维度上都优于其他方案——最佳选择取决于具体的应用场景和设计目标。
